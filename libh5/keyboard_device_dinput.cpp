#include "keyboard_device_dinput.h"
...
namespace h5
{
	keyboard_device_dinput::keyboard_device_dinput() {}
	...
	int32_t keyboard_device_dinput::init(const application_window_windows *window)
	{
		if (!window)
		{
		...
		if (FAILED(di->CreateDevice(GUID_SysKeyboard, &device, nullptr)))
		{
	...
	int32_t keyboard_device_dinput::init(const application_window_windows *window, const guid &id, const std::wstring &name)
	{
		if (!window)
		{
			return ERRLVL_DINPUT_NULL_WINDOW;
	...
	int32_t keyboard_device_dinput::read()
	{		
		...
		{
			for (size_t i = 0; i < (size_t)key::max; ++i)
			{
				if (state[i] & 0x80)
				{
	...
	void keyboard_device_dinput::clear_state()
	{
		memset(state, 0, sizeof(state));
		memset(events, 0, sizeof(events));
	}

	bool keyboard_device_dinput::is_event(key key, event event)
	{
		return events[static_cast<size_t>(key)] == event;
	}
}
