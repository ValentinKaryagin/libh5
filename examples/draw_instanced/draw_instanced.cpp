...
int main()
{
	...
	error_level = window.init(128, 128, 1280, 720, u("h5 draw instanced example"));
	...
	if (!error_level)
	{
		error_level = vs.load(u("draw_instanced.hlsl"), "vs");
		if (!error_level)
		{
	...
	if (!error_level)
	{
		error_level = ps.load(u("draw_instanced.hlsl"), "ps");
		if (!error_level)
		{
