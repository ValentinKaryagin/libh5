...
int main()
{
	...
	error_level = window.init(128, 128, 1280, 720, u("h5 global light example"));
	...
	if (!error_level)
	{
		error_level = vs.load(u("global_light.hlsl"), "vs");
		if (!error_level)
		{
	...
	if (!error_level)
	{
		error_level = ps.load(u("global_light.hlsl"), "ps");
		if (!error_level)
		{
	...
	if (!error_level)
	{
		error_level = texture_color.load(u("global_light.png"));
	}

	h5::shader_res texture_color_sr;
	if (!error_level)
	{
		error_level = render.create_shader_res(&texture_color, &texture_color_sr);
		if (!error_level)
		{
			render.set_pixel_shader_res(&texture_color_sr, 0);
		}
	}
