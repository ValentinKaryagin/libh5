	...
	if (getDebugDrawer())
	{
		...
		if (drawConstraints)
		{
			...
			for (int b = 0; b < m_multiBodies.size(); b++)
			{
				...
				{
					getDebugDrawer()->drawTransform(bod->getBaseWorldTransform(), 0.1f);
				}
				...
				for (int m = 0; m < bod->getNumLinks(); m++)
				{
					...
					if (mode & btIDebugDraw::DBG_DrawFrames)
					{
						getDebugDrawer()->drawTransform(tr, 0.1f);
					}
					//draw the joint axis
					if (bod->getLink(m).m_jointType == btMultibodyLink::eRevolute)
					{
						btVector3 vec = quatRotate(tr.getRotation(), bod->getLink(m).m_axes[0].m_topVec) * 0.1f;
					...
					{
						btVector3 vec = quatRotate(tr.getRotation(), bod->getLink(m).m_axes[0].m_bottomVec) * 0.1f;
